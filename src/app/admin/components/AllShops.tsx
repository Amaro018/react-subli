...
function ShopHistoryRow({ shop, index }: { shop: ShopType; index: number }) {
  ...
  const handleOpenDetails = useCallback((shop: ShopType) => {
      ...
      <tr className="hover:bg-gray-50 transition-colors">
        <td className="px-4 sm:px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
          <div className="flex items-center gap-2">
            ...
            <IconButton aria-label="expand row" size="small" onClick={toggleHistory}>
...
export default function AllShops() {
  ...
  const filteredShops = sortedShops.filter((shop: ShopType) => {
    ...
    const matchesSearch =
      ...
      (shop.contact || "").toLowerCase().includes(search)
